...
# #### Generate Conformed - Dim Domain
# ---
# Notebook to perform the creation of Dim Domain, storing the table in the `Conformed` Lakehouse as delta format
# 
# 
...
# read cleansed lakehouse
df_dicts = read_all_required_dataframes(target_table_configs['required_tables'], target_schema, trigger_time)
df = df_dicts['push_csc_domain_name']

# add primary, foreign and attributes keys
df = df \
    .transform(lambda df: add_hash_key(df, primary_key, ['DomainName'], globals()['csc']['name'])) \
    .transform(lambda df: add_hash_key(df, 'RegistrantKey', ['RegLastName'], globals()['csc']['name']))
