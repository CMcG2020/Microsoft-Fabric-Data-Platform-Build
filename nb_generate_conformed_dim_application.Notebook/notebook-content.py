...
# #### Generate Conformed - Dim Application
# ---
# Notebook to perform the creation of Dim Application, storing the table in the `Conformed` Lakehouse as delta format
# 
# 
...
df_application = df_dicts['Application'] \
    ...
    .withColumn('ApplicationStatus', F.when(F.col('Status') == 'ACTIVE', F.lit(True)).otherwise(F.lit(False))) \
...
# Note - not required to bring in hash email from pull user given these will be overridden to 'UNKNOWN'
...
for row in personal_schema:
  df_cmdb_ci_appl = df_cmdb_ci_appl.withColumn(row['field'], F.lit(None).cast(row['dataType']))
df_cmdb_ci_appl =  df_cmdb_ci_appl.transform(lambda df: fill_unknown_columns(df, [primary_key] + metadata_fields))


# union both dataframe, add IsMaster, re-order and add attributes hash
...
# add IsMaster field and remove key values identified as causing duplicates
df = df \
    .withColumn('IsMaster', F.when(F.col('SourceSystem') == globals()['lean_ix']['name'], F.lit(True)).otherwise(F.lit(False))) \

df = df.where(~(df.ApplicationKey.isin([-849862343517725175, 5896695335020484737])))
##### END TRANSFORMATION #####
