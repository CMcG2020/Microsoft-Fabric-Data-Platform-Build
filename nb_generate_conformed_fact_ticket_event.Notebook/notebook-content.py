...
# #### Generate Conformed - Fact Ticket Event
# ---
# Notebook to perform the creation of Fact Ticket Event, storing the table in the `Conformed` Lakehouse as delta format
# 
# 
...
total_count = df_dicts['task_sla'].count()
if total_count == 0:
...
df = df_dicts['task_sla'] \
    .withColumn("EventType", F.lit('SLA')) \
    .transform(lambda df: add_hash_key(df, 'TicketKey', ['Task'], globals()['service_now']['name'])) \
    .transform(lambda df: add_hash_key(df, 'AssignmentGroupKey', ['UInitialAssignmentGroup'], globals()['service_now']['name'])) \
    .transform(lambda df: add_hash_key(df, 'SupplierKey', ['uInitialVendor'], globals()['service_now']['name'])) \
    .transform(lambda df: add_hash_key(df, 'SupplierBreachedNameKey', ['UBreachedWithVendor'], globals()['service_now']['name'])) \
    .transform(lambda df: conformed_select_alias_and_cast(df, target_schema, 'task_sla', 'after')) \
    .transform(lambda df: transform_time_to_minutes(df, "BusinessDuration", "BusinessDuration")) \
    .select([F.col(f['field']).cast(f['dataType']) for f in target_schema]) \
    .transform(lambda df: add_hash_key(df, attributes_key, attributes))

# add partitions
df = df.withColumns({"Year": F.year("StartDateTime"), "Month": F.month("StartDateTime")})
##### END TRANSFORMATION #####
...
#Run PII Data only if df_privacy exists
if df_privacy:
    try:
