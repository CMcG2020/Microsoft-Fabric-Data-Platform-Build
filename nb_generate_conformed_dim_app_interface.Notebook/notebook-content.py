...
# #### Generate Conformed - Dim App Interface
# ---
# Notebook to perform the creation of Dim App Interface, storing the table in the `Conformed` Lakehouse as delta format
# 
# 
...
%run nb_schema_dim_app_interface
...
df = df_dicts['Interface'] \
    .filter(F.col('Type') == 'Interface') \
    .transform(lambda df: add_hash_key(df, target_table_configs['primary_key'], ['Name'], globals()['lean_ix']['name'])) \
