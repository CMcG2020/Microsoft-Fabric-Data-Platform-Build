...
# #### Generate Conformed - Fact Contract
# ---
...
df = df_dicts['contracts'] \
  .transform(lambda df: conformed_select_alias_and_cast(df, target_schema, 'contracts', 'before')) \
  .transform(lambda df: add_hash_key(df, 'ContractKey', ['Id'], globals()['coupa']['name'])) \
  .transform(lambda df: add_hash_key(df, 'SupplierKey', ['SupplierName'], globals()['coupa']['name'])) \
  .transform(lambda df: add_hash_key(df, 'ApproverKey', ['CustomFieldsContractApprover'])) \
  .withColumn('ContentGroups', F.translate('ContentGroups', '[]', '')) \
