...
# #### Generate Cleansed - CLIENT_DATA
# ---
# Generic notebook to perform cleansing / enriching activities for CLIENT_DATA data derived from the `Sourced` Lakehouse, before storing the cleansed / enriched data within the `Cleansed` Lakehouse as parquet format.
# 
# 
...
%run nb_schema_sourced_client_data
...
source_configs = globals()['client_data']
...
# Convert date strings to DateType
  ...
  cleansed_df = cleansed_df \
      .withColumn('Registration Date', F.to_date(F.col('Registration Date'), date_format)) \
      .withColumn('Expiry Date', F.to_date(F.col('Expiry Date'), date_format))
