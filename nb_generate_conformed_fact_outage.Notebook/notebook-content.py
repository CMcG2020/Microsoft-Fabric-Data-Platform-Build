...
# #### Generate Conformed - Fact Outage
# ---
# Notebook to perform the creation of Fact Outage, storing the table in the `Conformed` Lakehouse as delta format
# 
# 
...
total_count = df_dicts['cmdb_ci_outage'].count()
if total_count == 0:
    print(f'No outage raised in the last 24 hours, ending notebook')
    sys.exit(0)
##### END VALIDATE COUNTS #####
...
df = df_dicts['cmdb_ci_outage'] \
  .transform(lambda df: add_hash_key(df, primary_key, ['Number'], globals()['service_now']['name'])) \
  .transform(lambda df: add_hash_key(df, 'TicketKey', ['TaskNumber'], globals()['service_now']['name'])) \
  .transform(lambda df: add_hash_key(df, 'ApplicationKey', ['Application'], globals()['service_now']['name'])) \
  .transform(lambda df: add_hash_key(df, 'ChildServiceKey', ['CmdbCi'], globals()['service_now']['name'])) \
  .withColumn("OutageDuration", convert_duration_to_minutes(F.col("Duration"))) \
  .transform(lambda df: conformed_select_alias_and_cast(df, target_schema, 'cmdb_ci_outage', 'after')) \
