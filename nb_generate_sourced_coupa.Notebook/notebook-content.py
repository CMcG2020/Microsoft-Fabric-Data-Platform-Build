...
# #### Generate Sourced - External System
# ---
...
%run nb_schema_sourced_external_system
...
source_configs = globals()['external_system']
...
# retrieve only required fields from external system
required_fields = [row['from'] if not row.get('nested') else {row['from'].replace("-", "_"): [x['name'] for x in row['nested']['sub_fields']]} for row in source_schema]

# extract data
external_client = ExternalSystemReader(client_id, api_token, table_name)
response_data = external_client.make_api_call(required_fields)

# write to sourced lakehouse
write_parquet_to_lakehouse(
    response_data, global_configs['sourced_lh_name'],
